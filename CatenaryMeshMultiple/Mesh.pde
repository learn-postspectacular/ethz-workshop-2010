  ...
  ParticleMesh(VerletPhysics physics, int gridSize, int restLength, float strength) {
    ...
    for(int y=0,idx=0; y<gridSize; y++) {
      for(int x=0; x<gridSize; x++) {
        ...
        particles.add(p);
        if (x>0) {
          VerletSpring s=new VerletSpring(p,physics.particles.get(idx-1),restLength,strength);
          physics.addSpring(s);
        }
        if (y>0) {
          VerletSpring s=new VerletSpring(p,physics.particles.get(idx-gridSize),restLength,strength);
          physics.addSpring(s);
        }
        idx++;
      }
